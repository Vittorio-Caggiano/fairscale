    ...
    def forward(self, input: TensorOrTensors) -> TensorOrTensors:  # type: ignore
        ...
        if self.group and not self.final_stage:
            ...
            if self.pipelined_backward:
                ...
                output = YackityHack.apply(output, batches, phony, True)  # self.retain_graph)
            else:
                output = microbatch.gather(batches)
...
class YackityHack(torch.autograd.Function):
    @staticmethod
    # type: ignore
    ...
    def backward(ctx, *grads) -> Tuple:
        ...
        with torch.no_grad():
            test_parallel_randoms_error = (
                "[W variable.cpp:464] Warning: Output 0 of YackityHackBackward is a view and is being modified inplace."
                ...
                " version 1.6. You can remove this warning by cloning the output of the custom Function. (function"
